function [color_score, count, average_color_diff,variance] = ...
    calculate_color_task3_solution(cropped_img_cl,cropped_img_mask,color_table,threshold,plot_image)
...
%          calculate_color_task1_solution(cropped_img_cl,cropped_img_mask,color_table,threshold,plot_image));
% Date: October,23, 2016; RK

%% 1) Extract RGB color channels
...
num_pix_in = length(inside);
num_color = size(color_table,1);
eucl_diff = NaN(num_color,1);
count = zeros(num_color,1);

for i=1:num_pix_in
    for j=1:num_color
        eucl_diff(j)=norm([R(inside(i)) G(inside(i)) B(inside(i))]-color_table(j,:));
    end
    [~,ind_min] = min(eucl_diff);
...
dummy = count >= threshold;
color_score = sum(dummy);
...
%% 5) Calculate the variance of the R,G,B channels inside the lesion
variance = [var(R(inside)),var(G(inside)),var(B(inside))];
